...
class Model(object):
    ...
    def update_pi(self, R):
        ...
        ######################## Your code here #########################
        #A_k defined in the above expected joint log probability
        A_k= self.prior.a_mix
        #given in the question
        K = R.shape[1]

        return (R.sum(axis=0)+A_k-1)/(R.sum(axis=1).sum()+(K*A_k)-K)
    ...
    def update_theta(self, X, R):
        ...
        ######################## Your code here #########################
        #define all paramaters needed
        a=self.prior.a_pixels
        b=self.prior.b_pixels

        top= np.dot(R.T,X)+(a-1)
        bottom_1 = np.dot(R.T,(1-X))+b-1
        bottom_2= top
    ...
    def compute_posterior(self, X, M=None):
        ...
        log_p_z_x=log_pi+np.dot(X_obs,np.log(self.params.theta).T)+np.dot((1. -X_obs),np.log(1. -self.params.theta).T)
    ...
    def posterior_predictive_means(self, X, M):
        ...
        ######################## Your code here #########################
        pass
        post=self.compute_posterior(X,M)
        post_pred_mean=np.dot(post,self.params.theta)
