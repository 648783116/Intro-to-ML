...
def printTree(clf_final_tree,features):
    from sklearn import tree
    tree.export_graphviz(clf_final_tree,out_file='tree.dot',feature_names=features,filled=True)
    from sklearn.externals.six import StringIO
    import pydot
    dot_data = StringIO()
    tree.export_graphviz(clf_final_tree, out_file=dot_data,feature_names=features,filled=True)
    graph = pydot.graph_from_dot_data(dot_data.getvalue())
    a = graph[0].write_pdf("tree.pdf")
    from IPython.display import Image
    import os
    return Image(filename=os.getcwd()+'/tree.png')

printTree(clf_final_tree,features)
